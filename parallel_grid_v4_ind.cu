...
if(MM >= 0){
	if(mm > 0){
		// if(((x1[j] <= 0 and x2[j] >= 0)and(y1[j]<=0 and y2[j] >= lmax))or((x1[j] <= l0 and x2[j] >= l0)and(y1[j]<=0 and y2[j] >= lmax)))
		// 	{
		// 		// device_grid[i*stride[0]+j] = i*stride[0]+j;
	  ...
	  // device_grid[blockDim.x*gridDim.x*stride[0]*3 + i*stride[0]+j] = y2[j];
		...
		// device_grid[i*stride[0]+j] = x1[j];
		 device_grid[blockDim.x*gridDim.x*stride[0] + i*stride[0]+j] = i*stride[0]+j;
		// device_grid[blockDim.x*gridDim.x*stride[0]*2 + i*stride[0]+j] = y1[j];
		// device_grid[blockDim.x*gridDim.x*stride[0]*3 + i*stride[0]+j] = y2[j];
	}
...
int main(){
  ...
  std::chrono::time_point<std::chrono:: high_resolution_clock> start, end;
	...
	cudaMallocManaged(&device_grid, n_of_blocks * n_of_threads * 2 * sizeof (double));
...
for(int i = 0; i < n_of_blocks*n_of_threads; i++){
  cout<<"["<<r1.x1+(int(device_grid[i])%(n_of_threads))*approximation<<":"<<r1.x1+(int(device_grid[i])%(n_of_threads) + 1)*approximation<<"]:";
  cout<<"["<<r1.y1+(int(device_grid[i])/(n_of_threads))*approximation<<":"<<r1.y1+(int(device_grid[i])/(n_of_threads) + 1)*approximation<<"]\n";
}
  cout<<"_+_+_+_\n";
	//INTERNAL
for(int i = 0; i < n_of_blocks*n_of_threads; i++){
  cout<<"["<<r1.x1+(int(device_grid[i+n_of_blocks*n_of_threads])%(n_of_threads))*approximation<<":"<<r1.x1+(int(device_grid[i+n_of_blocks*n_of_threads])%(n_of_threads) + 1)*approximation<<"]:";
  cout<<"["<<r1.y1+(int(device_grid[i+n_of_blocks*n_of_threads])/(n_of_threads))*approximation<<":"<<r1.y1+(int(device_grid[i+n_of_blocks*n_of_threads])/(n_of_threads) + 1)*approximation<<"]\n";
}
