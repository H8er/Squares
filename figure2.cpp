...
main(){
	int l = 8;
	int l0 = 5;
	double approximation = sqrt((0.01*0.01+0.01*0.01));
...
for(int i = 0;i<InternalRectangles.size();i++){
	//cout<<"\n_________"<<i<<"_________\n";
	//cout<<"["<<InternalRectangles[i].x1<<":"<<InternalRectangles[i].x2<<"]:[";	cout<<InternalRectangles[i].y1<<":"<<InternalRectangles[i].y2<<"]\n";
}
for(int i = 0;i<BoundRectangles.size();i++){
//InternalRectangles.push_back(BoundRectangles[i]);
}
//sort(InternalRectangles.begin(),InternalRectangles.end(),veccomp);
//merge_rect(InternalRectangles);
