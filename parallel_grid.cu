...
int main(){
  ...
  cout<<fixed;
	...
	cudaMalloc((void **) &device_grid, n_of_blocks * n_of_threads * 4 * sizeof (double));
...
for(int i = 0; i<n_of_blocks*n_of_threads;i++){
	if((host_grid[i]!=host_grid[i+n_of_blocks*n_of_threads]) and (host_grid[i+n_of_blocks*n_of_threads*2]!=host_grid[i+n_of_blocks*n_of_threads*3])){
	cout<<"["<<host_grid[i]<<":"<<host_grid[i+n_of_blocks*n_of_threads]<<"]:["<<host_grid[i+n_of_blocks*n_of_threads*2]<<":"<<host_grid[i+n_of_blocks*n_of_threads*3]<<"]\n";
}
}
